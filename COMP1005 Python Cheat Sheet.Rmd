...
$\textit{set}$.add($\textit{x}$) & Adds \textit{x} to an existing set \\
$\textit{set}$.update($\textit{x}$) & Adds variable \textit{x} holding multiple items to an existing set \\
...
any($\textit{item}$ in $\textit{set1}$ for $\textit{item}$ in $\textit{set1}$) & Like intersection, but gives a boolean \\
...
set/list($\textit{df}$['$\textit{key}'$]) & Creates a set or list from column entries \\
...
* Indexing/subsetting:
  ...
  * Columns for all rows
    ...
    * Specific columns - $\textit{df }.loc[:, [\textit{keylist}]]$
  ...
  * Rows and columns
    ...
    * Specific rows and columns - $\textit{df }loc[[\textit{rowlist}], [\textit{keylist}]]$
